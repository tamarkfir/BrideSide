...
export type VerifiedSong = {
  name: string;
  artist: string;
  /** שורות אמיתיות מהשיר — null אם לא נמצאו */
  excerpt: string | null;
...
עבור כל שיר שמוזכר בפרופיל — מצאי שורות אמיתיות ומדויקות מהשיר (1–4 שורות משמעותיות) באמצעות חיפוש.
אם לא הצלחת לאמת את המילים המדויקות — ציינו excerpt: null. **אסור בהחלט להמציא מילים.**

**2. הצעת 2–3 שירים נוספים:**
הציעי שירים שמתאימים **במיוחד לזוג הזה** — לפי הרקע שלהן, הערכים, האווירה, ולא שירים גנריים לחתונה.
עבור כל הצעה — מצאי גם קטע אמיתי מהשיר (אם אפשר), והסבירי למה הוא מתאים לזוג הזה דווקא.

**החזירי JSON בפורמט הזה (רק ה-JSON, בלי טקסט נוסף):**
{"songs": [
  {"name": "שם השיר", "artist": "שם האמן", "excerpt": "שורות אמיתיות מהשיר או null", "why": "למה מתאים לזוג הזה", "suggested": false},
  {"name": "שיר שאני מציעה", "artist": "אמן", "excerpt": "שורות אמיתיות", "why": "הסבר מפורט ואישי", "suggested": true}
]}

חשוב: כל excerpt חייב להיות מילות שיר אמיתיות שמצאת בחיפוש. אם לא מצאת — החזירי null ולא תמציאי.`;
}
...
export function buildCeremonyPrompt(state: SessionState, verifiedSongs: VerifiedSong[] = []): string {
  ...
  const songsSection = verifiedSongs.length > 0
    ? `\n\nשירים שנחקרו ואומתו:\n${verifiedSongs.map(s => {
        const tag = s.suggested ? "[הצעה חדשה]" : "[שיר שהזוג בחרו]";
        const excerpt = s.excerpt ? `\n  קטע אמיתי: "${s.excerpt}"` : "\n  (לא נמצאו מילים מאומתות — אזכרי בשם בלבד)";
        const why = s.why ? `\n  מדוע מתאים: ${s.why}` : "";
...
- שירים ומילים: **כלל מוחלט ובלתי-ניתן-לוויתור** — ציטוט של מילות שיר מותר **רק אם** מופיע קטע מאומת בסעיף "שירים שנחקרו ואומתו" למעלה. צטטי אותו מילה במילה ללא שינוי. אם לשיר יש "(לא נמצאו מילים מאומתות)" — **אסור לצטט שורות ממנו**; הזכירי אותו בשמו ותאר מה הוא מייצג לזוג. לשירים שהזוג הדביקו שורות ספציפיות בשדה "שורות ספציפיות" — צטטי אותן במדויק. אסור בהחלט להמציא, לנחש, או "לשחזר" שורות שלא קיבלת — גם אם את חושבת שאת זוכרת אותן.
...
- ציטוטים מאומתים מתוך שירים יופיעו כ-kind: quote או reading, עם attribution למקור (שם השיר ואמן). התייחסויות לשירים ללא מילים מאומתות יופיעו כ-kind: flow — ללא ציטוט ישיר.
