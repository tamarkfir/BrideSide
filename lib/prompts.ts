...
עבור כל שיר שמוזכר בפרופיל — חפשי ברשת ונסי לאמת 1–4 שורות אמיתיות מהשיר.

**כללי אימות מחמירים (excerpt מותר רק אם כל התנאים מתקיימים):**
- מצאת עמוד מילים אמיתי (אתר מילים/לֵירִיקְס) שבו מופיע **גם שם השיר וגם שם האמן המדויק** שביקשו. אם מצאת שיר באותו שם אבל של אמן אחר — זה לא אותו שיר, החזירי excerpt: null.
- שמות שירים בעברית חוזרים על עצמם (למשל "ביחד", "יחד", "אהבה") — אם יש ספק לאיזה שיר/אמן השורות שייכות, או שלא הצלחת לקשר את השורות לאמן הספציפי הזה בוודאות — החזירי excerpt: null.
- העתיקי את השורות **מילה-במילה, אות-באות** מהמקור. **אסור לנסח מחדש, לתקן, לקצר, להשלים מהזיכרון או "לשחזר".** אם את לא מעתיקה ממקור שראית בחיפוש — החזירי null.
- כש-excerpt לא null — חובה למלא את שדה "source" עם ה-URL המדויק שממנו העתקת. אין URL → אין excerpt.

ברירת המחדל היא **null**. עדיף שיר ללא ציטוט מאשר ציטוט שגוי. **אסור בהחלט להמציא מילים.**
...
- אותם כללי אימות מחמירים חלים על כל excerpt (אמן מדויק, העתקה מילה-במילה, source חובה, אחרת null). הסבירי למה כל שיר מתאים לזוג הזה דווקא.
...
export function buildCeremonyPrompt(state: SessionState, verifiedSongs: VerifiedSong[] = []): string {
  ...
  // הצעות של Gemini תמיד עם קטע מאומת (אחרת סוננו). שיר שהזוג בחרו עשוי להגיע
  // ללא קטע מאומת — ואז מזכירים אותו בשם ובמשמעות בלבד, בלי ציטוט.
  const songsSection = verifiedSongs.length > 0
    ? `\n\nשירים לטקס (רק אלה — אין שירים אחרים):\n${verifiedSongs.map(s => {
        const tag = s.suggested ? "[הצעה חדשה - מאומתת]" : "[שיר שהזוג בחרו]";
        const excerpt = s.excerpt
          ? `\n  קטע אמיתי (צטטי מילה-במילה): "${s.excerpt}"`
          : "\n  (לא נמצאו מילים מאומתות — הזכירי בשם ובמשמעות בלבד, ללא ציטוט שורות)";
        const why = s.why ? `\n  מדוע מתאים: ${s.why}` : "";
...
- שירים ומילים: **כלל מוחלט ובלתי-ניתן-לוויתור** — השתמשי **אך ורק** בשירים שמופיעים בסעיף "שירים לטקס" למעלה. **אסור להזכיר שום שיר אחר.** ציטוט מילות שיר מותר **רק** מתוך "קטע אמיתי" שמופיע שם — מילה-במילה ללא שינוי. לשיר שמסומן "(לא נמצאו מילים מאומתות)" — **אסור לצטט ממנו שורות**; הזכירי אותו בשמו ובמה שהוא מייצג לזוג בלבד. לשירים שהזוג הדביקו שורות ספציפיות בשדה "שורות ספציפיות" — צטטי אותן במדויק. אסור בהחלט להמציא, לנחש, או "לשחזר" שורות או שירים שלא קיבלת — גם אם את חושבת שאת זוכרת אותם.
...
- ציטוטים מאומתים מתוך שירים יופיעו כ-kind: quote או reading, עם attribution למקור (שם השיר ואמן). אם אין אף שיר מאומת — פשוט אל תכללי קטע שירים בטקס.
